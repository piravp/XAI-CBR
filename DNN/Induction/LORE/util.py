...
def recognize_features_type(df, class_name):
    """  Figure out which datatype in each column, and return dictionary that track this. """    
...
def set_discrete_continuous(features, type_features, class_name, discrete=None, continuous=None):
    """ Return which columns are continous and which are discrete """
    if discrete is None and continuous is None: # both None
        discrete = type_features['string'] # discrete type is a string.
    ...
    continuous = [f for f in continuous if f != class_name] # set all columns continous that are not equail to class_name in continous.
    return discrete, continuous


def label_encode(df, columns, label_encoder=None):
    """ Encode the label columns using scikit preprocessing, from non-numeric labels to numeric labels. """
    df_le = df.copy(deep=True)
    new_le = label_encoder is None
    label_encoder = dict() if new_le else label_encoder
    for col in columns:
        if new_le:
            le = LabelEncoder() # preprocessing labelEncoder
            df_le[col] = le.fit_transform(df_le[col]) # fit the data column. Turn everything into numbers.
...
def label_decode(df, columns, label_encoder):
    """ decode datafram labels, with inverse transform function """
    df_de = df.copy(deep=True) # copy dataframe
    for col in columns: # check every feature
        le = label_encoder[col]
...
def build_df2explain(bb, X, dataset):
    """ build dataframe to explain """
    columns = dataset['columns']
    features_type = dataset['features_type']
    discrete = dataset['discrete']
    label_encoder = dataset['label_encoder']
    
    y = bb.predict(X) # Generate predictions from black-box,X values
