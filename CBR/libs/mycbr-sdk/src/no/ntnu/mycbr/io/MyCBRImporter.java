...
import no.ntnu.mycbr.core.similarity.TaxonomyNode;
    ...
    private class FunctionImporter extends DefaultHandler {
        ...
        public void startElement(String namespaceURI, String localName,
            ...
            if ("SMFunction".equals(qName)
                ...
                f.setSymmetric(false);
                MultipleConfig.MainType mt = null;
                MultipleConfig.Reuse r = null;
                MultipleConfig.Type t = null;
                ...
                if (ONEORMULTI_MODE.equals("10")) { // ONE
                    if (SINGLESIM_MODE.equals("30")) { // BEST
                        mt = MultipleConfig.MainType.BEST_MATCH;
                    } else { // WORST
                        mt = MultipleConfig.MainType.WORST_MATCH;
                    }
                    r = MultipleConfig.Reuse.NONE;
                    t = MultipleConfig.Type.NONE;
                } else { // Multi
                    if (MULTISELECTION_MODE.equals("20")) { // by query
                        mt = MultipleConfig.MainType.PARTNER_QUERY;
                    } else if (MULTISELECTION_MODE.equals("21")) { // by case
                        mt = MultipleConfig.MainType.PARTNER_CASE;
                    } else {
                        mt = MultipleConfig.MainType.PARTNER_MAX;
                    }

                    if (REUSE_MODE.equals("40")) {
                        r = MultipleConfig.Reuse.REUSE;
                    } else {
                        if (NOMATCH_MODE.equals("45")) {
                            r = MultipleConfig.Reuse.ZERO_SIM;
                        } else {
                            r = MultipleConfig.Reuse.IGNORE;
                        }
                    }

                    if (TOTAL_MODE.equals("50")) {
                        t = MultipleConfig.Type.AVG;
                    } else if (TOTAL_MODE.equals("51")) {
                        t = MultipleConfig.Type.MAX;
                    } else {
                        t = MultipleConfig.Type.MIN;
                    }
                }
