...
class Datamanager():
    ...
    def __init__(self, reduce=False, dataset=None,
                 in_mod="normal", out_mod="one-hot", train_frac=0.9,
                 freq_lim=3):  # 90% to be used as training, 10 % testing/val
    ...
    def adults(self):
        ...
        columns_to_remove = ["fnlwgt", "education-num"]         # column names we do not want to keep
        ...
        label_map = {'<=50K': 'Less than $50,000', '>50K': 'More than $50,000', '<=50K.': 'Less than $50,000',
                     '>50K.': 'More than $50,000'}

        cap_gain_map = {'0': 'None', '1': 'Low', '2': 'High'}

        # Path to data folder.
        filename = pathlib.Path(__file__).parents[0] / "data"
        ...
        disc = lime.lime_tabular.EntropyDiscretizer(df.values,
                                                    ...
                                                    labels=labels)
        """
        transformations = {  # Mapping columns to dict maps or functions.
        ...
        # We don't bother balancing the dataset.
    ...
    #         [misc.int_to_one_hot_vector(int(item), size=self.classes, zero_offset=1) for item in data_targets])
